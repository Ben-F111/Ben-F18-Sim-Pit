...
//#define NUM_LEDS_CHA 1// Open Hornet UPPER INST PANEL Backlight PCBS Version 2.0 by L Walker
#define DATA_PIN_CHA 12// ARDUINO PIN that UPPER INST PANEL is connected to // CHANGE AS REQUIRED
...
//
#define NUM_LEDS_CHF 400 // Generic Backlight loop - SPARE (100 LEDS) Change as required
...
int FASTled; // fastLED.show() Update Flag
...
static int AAOn;
static int SpinOn;
if (emcON != EmcON || disON != DISOn || rdyON != RDYOn || agON != AGOn || aaON != AAOn ||spinOn != SpinOn)
  {  FASTled = HIGH;
EmcON = emcON;
DISOn = disON;
RDYOn = rdyON;
...
if (conDim != prevConDim || instDim != prevInstDim)
  {  FASTled = HIGH;
 prevConDim = conDim;
 prevInstDim = instDim;}
*/
