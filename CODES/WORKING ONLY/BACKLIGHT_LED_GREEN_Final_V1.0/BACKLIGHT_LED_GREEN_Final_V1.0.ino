...
instDim = map(newValue, 0, 65535, 10, 150);
instDimH = map(newValue, 0, 65535, 60, 140);
}
...
cautDim = map(newValue, 0, 65535, 20, 2000);
...
//TEST ALL LEDS -RED-BLUE-BLACK-GREEN
   ...
   fill_solid(leds_D, NUM_LEDS_CHD, CRGB::Red); // Set all Console LEDs to ##GREEN##. 
    FastLED.setBrightness(50); // SET CONSOLE PANELS GREEN BRIGHTNESS
    FastLED.show();
    delay(2000);
...
void loop() {
  ...
  if (instDim != prevInstDim)
  {
    prevInstDim = instDim;
    for (i = 0; i < 50; i++) 
    // SPIN Green LEDS CHANNEL A
    leds_A[spinPanel[i]] = CHSV( 0, 255, instDimH); // SET SPIN PANEL GREEN BRIGHTNESS
        for (j = 0; j < 78; j++) 
    // EMC-JETT Green LEDS CHANNEL D
    leds_D[emcJettPanel[j]] = CHSV( 0, 255, instDimH); // SET SPIN PANEL GREEN BRIGHTNESS
    // generic Consoles below "GREEN" CHANNEL 
    fill_solid(leds_B, NUM_LEDS_CHB, CRGB::Red); // Set all Console LEDs to ##GREEN##. 
    fill_solid(leds_C, NUM_LEDS_CHC, CRGB::Red); // Set all Console LEDs to ##GREEN##. 
