...
sendCommand("bt63.pic0=24"); // UV
 ...
 //Chiller OUTPUT ********************************************************
  ...
  if (chillsw==LOW && waterchangesw==LOW) {  
sendCommand("bt62.pic0=27"); // Chiller
digitalWrite(chillPin, HIGH);
    } else{
sendCommand("bt62.pic0=24"); // Chiller
...
if ((waterchangesw == HIGH && drainsw == HIGH) && (sumpwchg == LOW || sumpempty == LOW)) {
digitalWrite(drainsump, HIGH); // Sump Drain bit on
dbSerialPrintln("drainsump OK"); 
}
else {
  dbSerialPrintln("drainsump NOT OK"); 
  digitalWrite(drainsump, LOW); // Sump Drain bit off
}
//**************************************************************************

buttonState = digitalRead(drainsump);
   ...
   if (buttonState != lastButtonState) {
     ...
     if (buttonState == HIGH) {
       ...
       // wend from off to on:
       dbSerialPrintln("drainsump 22 OK"); 
     } else {
      dbSerialPrintln("drainsump 22 NOT OK"); 
     }
 ...
 /*if (drainsump==HIGH) {
  ...
  dbSerialPrintln("drainsump part 2 OK");  
 } 
if (drainsump==LOW) {
 digitalWrite(drainPin, LOW);
   dbSerialPrintln("drainsump part 2 Not OK");  
 }*/
